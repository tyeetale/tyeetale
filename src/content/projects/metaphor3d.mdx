...
We explored different interface patterns first. A Discord bot? An email system where you send queries and get 3D assets back? We racked our brains before landing on a web app. The target was indie game developers and [Roblox creators](https://create.roblox.com) who spent disproportionate money on custom 3D assets or settled for generic stock.

We built an MVP using text-to-3D fusion models, wrapped in a [Three.js](https://threejs.org) viewer where you describe what you want and see generated meshes rendered in the browser. Zhizhuo handled the ML pipeline, I owned the web interface, the viewer, and the overall design.
